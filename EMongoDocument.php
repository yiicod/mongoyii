...
class EMongoDocument extends EMongoModel
{
    ...
    public function __construct($scenario = 'insert')
    {
        ...
        if (isset($this->_id)) {
            throw new CException('"_id" this is reserved attr');
        }
...
//    {
//        return $value instanceof MongoDB\BSON\ObjectID ? $value : new MongoDB\BSON\ObjectID($value === null ? uniqid() : $value);
//    }
//
//    public function setMongoId($value = null)
//    {
//        $this->_id = $value;
//    }
     ...
     * @param string|MongoDB\BSON\ObjectID $value
     * @return MongoDB\BSON\ObjectID
     */
    public function getPrimaryKey($value = null)
    {
        if ($value === null && $this->_id instanceof MongoDB\BSON\ObjectID) {
            $pk = $this->_id;
        } elseif (is_string($value) && strlen($value)) {
            $pk = new MongoDB\BSON\ObjectID($value);
     ...
     */
    public function populateRecords(array $data, $callAfterFind = true, $index = null, $partial = false)
    {
        $records = [];
        foreach ($data as $attributes) {
            if (($record = $this->populateRecord($attributes, $callAfterFind, $partial)) !== null) {
                if ($index === null) {
                    $records[] = $record;
                } else {
    ...
    public function saveAttributes($attributes)
    {
        ...
        return $this->lastResult = $this->updateByPk($this->getPrimaryKey(), ['$set' => $values]);
    }
...
//        }
        ...
        if ($this->getDbConnection()->enableProfiling) {
            Yii::beginProfile(
                'extensions.MongoYii.EMongoDocument.query.' . $this->collectionName() . '.insert(' . '{$document:' . json_encode($document) . '})', 'extensions.MongoYii.EMongoDocument.insert'
            );
        }

        $this->lastResult = $this->getCollection()->insertOne($document, $this->getDbConnection()->getDefaultWriteConcern());
        ...
        if ($this->lastResult) {
            ...
            $this->setPrimaryKey($this->lastResult->getInsertedId());
            return true;
        }
        return false;
    }
...
//                }
//            }
//            var_dump(array($this->primaryKey() => $this->getPrimaryKey(), $this->versionField() => $version), $attributes, array('multiple' => false));die;
            $this->lastResult = $this->updateAll(
                [$this->primaryKey() => $this->getPrimaryKey(), $this->versionField() => $version], $attributes, ['multiple' => false]
            );
        } else {
//            if ($partial === true) { // If this is a partial docuemnt we use $set to replace that partial view
            $attributes = ['$set' => $attributes];
//            }
            $this->lastResult = $this->updateByPk($this->getPrimaryKey(), $attributes);
        }

        if ($this->versioned() && $this->lastResult->getModifiedCount() <= 0) {
            return false;
        }
        $this->afterSave();
        return true;
    ...
    public function findOne($criteria = [], $fields = [])
    {
        ...
        Yii::trace(
            'Executing findOne: ' . '{$query:' . json_encode($query) . ',$project:' . json_encode($project) . '}', 'extensions.MongoYii.EMongoDocument'
        );

        if (
            $this->getDbConnection()->queryCachingCount > 0 && $this->getDbConnection()->queryCachingDuration > 0 && $this->getDbConnection()->queryCacheID !== false && ($cache = Yii::app()->getComponent($this->getDbConnection()->queryCacheID)) && ($cacheKey = 'yii:dbquery' . $this->getDbConnection()->server . ':' . $this->getDbConnection()->db . ':' . $this->getDbConnection()->getSerialisedQuery($query, $project) . ':' . $this->getCollection()) && ($result = $cache->get($cacheKey)) !== false
        ) {
            ...
            if ($this->getDbConnection()->enableProfiling) {
                Yii::beginProfile(
                    'extensions.MongoYii.EMongoDocument.query.' . $this->collectionName() . '.findOne(' . '{$query:' . json_encode($query) . ',$project:' . json_encode($project) . '}' . ')', 'extensions.MongoYii.EMongoDocument.findOne'
                );
            }

            $record = $this->getCollection()->findOne($query, ['projection' => $project]);
            $record = $record === null ? null : iterator_to_array($record);
     ...
     * @param mixed $pk - String, MongoDB\BSON\ObjectID or array of strings or MongoDB\BSON\ObjectID values (one can mix strings and MongoDB\BSON\ObjectID in the array)
     * @param array|string[] $fields
     * @return EMongoCursor|EMongoDocument[]
     * @throws EMongoException
     */
    ...
    {
        if (is_string($pk) || $pk instanceof MongoDB\BSON\ObjectID) {
    ...
    protected function find($criteria = [], $fields = [])
    {
        ...
        //$options = [];
        if ($c !== []) {
            $builder = new EMongoQueryBuilder($this, $query, $project);
            if (isset($c['sort'])) {
                $builder->sort($c['sort']);
            }
            if (isset($c['skip'])) {
            ...
            $builder = new EMongoQueryBuilder($this, $criteria, $project);
        }

        $finded = $builder->queryAll();
        $results = $this->populateRecords($finded, true, null, $project === [] ? false : true);
    ...
    /**
     * Finds one by _id
     * @param string|MongoDB\BSON\ObjectID $_id
     * @param array|string[] $fields
     * @return EMongoDocument|null
     */
     ...
     * @param string|MongoDB\BSON\ObjectID $pk
     * @param array|string[] $fields
     * @return EMongoDocument|null
     */
    ...
    public function deleteByPk($pk, $criteria = [], $options = [])
    {
        ...
        if ($this->getDbConnection()->enableProfiling) {
            Yii::beginProfile(
                'extensions.MongoYii.EMongoDocument.query.' . $this->collectionName() . '.deleteByPk({$query:' . json_encode($query) . '})', 'extensions.MongoYii.EMongoDocument.deleteByPk'
            );
        }
        $result = $this->getCollection()->deleteOne($query, array_merge($this->getDbConnection()->getDefaultWriteConcern(), $options));
        if ($this->getDbConnection()->enableProfiling) {
            Yii::endProfile(
                'extensions.MongoYii.EMongoDocument.query.' . $this->collectionName() . '.deleteByPk({$query:' . json_encode($query) . '})', 'extensions.MongoYii.EMongoDocument.deleteByPk'
            );
    ...
    public function updateByPk($pk, $updateDoc = [], $criteria = [], $options = [])
    {
        ...
        if (YII_DEBUG) {
            ...
            // having to do the serialisation on the update doc normally.
            Yii::trace(
                'Executing updateByPk: {$query:' . json_encode($query) . ',$document:' . json_encode($updateDoc) . '}', 'extensions.MongoYii.EMongoDocument'
            );
        }
        if ($this->getDbConnection()->enableProfiling) {
            Yii::beginProfile(
                'extensions.MongoYii.EMongoDocument.query.' . $this->collectionName() . '.updateByPk({$query:' . json_encode($query) . ',$document:' . json_encode($updateDoc) . '})', 'extensions.MongoYii.EMongoDocument.updateByPk'
            );
        }
//        var_dump($updateDoc);die;
        $result = $this->getCollection()->updateOne(
            $query, $updateDoc, array_merge($this->getDbConnection()->getDefaultWriteConcern(), $options)
        );
    ...
    public function updateAll($criteria = [], $updateDoc = [], $options = ['multiple' => true])
    {
        ...
        if (YII_DEBUG) {
            ...
            Yii::trace(
                ...
                . ',$document:' . json_encode($updateDoc) . ',$options:' . json_encode($options) . '}', 'extensions.MongoYii.EMongoDocument'
            );
        }
        if ($this->getDbConnection()->enableProfiling) {
        ...
        $result = $this->getCollection()->updateMany($criteria, ['$set' => $updateDoc], $options);
    ...
    public function deleteAll($criteria = [], $options = [])
    {
        ...
        Yii::trace(
            'Executing deleteAll: {$query:' . json_encode($criteria) . '}', 'extensions.MongoYii.EMongoDocument'
        );
        if ($this->getDbConnection()->enableProfiling) {
            Yii::beginProfile(
                'extensions.MongoYii.EMongoDocument.query.' . $this->collectionName() . '.deleteAll({$query:' . json_encode($criteria) . '})', 'extensions.MongoYii.EMongoDocument.deleteAll'
            );
        }

        $result = $this->getCollection()->deleteMany($criteria, array_merge($this->getDbConnection()->getDefaultWriteConcern(), $options));
    ...
    public function search($query = [], $project = [], $partialMatch = false, $sort = [])
    {
                    ...
                    if ($partialMatch === true) {
                        ...
                        if (
                            !is_bool($value) && !is_array($value) && preg_match('/^([0-9]|[1-9]{1}\d+)$/' /* Will only match real integers, unsigned */, $value) > 0 && (
                                (PHP_INT_MAX > 2147483647 && (string)$value < '9223372036854775807') /* If it is a 64 bit system and the value is under the long max */ || (string)$value < '2147483647' /* value is under 32bit limit */
                            )
                        ) {
                            $value = (int)$value;
                        }
        ...
        return new EMongoDataProvider($this, ['criteria' => ['condition' => $query, 'project' => $project, 'sort' => $sort]]);
    }
    ...
    public function ensureIndexes($indexes)
    {
        foreach ($indexes as $index) {
            if (isset($index[0])) {
                $this->getCollection()->createIndex($index[0], isset($index[1]) ? $index[1] : []);
            } else {
                $this->getCollection()->createIndex($index, []);
            }
        }
        return true;
    }
