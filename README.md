...
This is update of the MongoYii extension.

Another active record handler for the Yii framework that supports MongoDB for PHP 7.
...
You will notice that I use a `EMongoClient`. This is a bit deceptive since it actually represents `MongoDB\Client` and `MongoDB` combined.  This means that whenever you call the magic `__call`
...
If you wish to call a function on the `MongoDB\Client` or `Mongo` class you will need to retrieve the connection object like so:
```php 
...
`find()` is really simple. It is essentially a 1-1 to the drivers own `find()` function and implements the same specifics. Just like the drivers edition, it also returns a array of EMongoDociument.
...
- `->find(/*...*/)` is basically the MongoDB drivers `find` method and returns a array of EMongoDocument or BSONArray
- `->sort()` is basically the MongoDB drivers `sort` method on the `EMongoQueryBuilder`
- `->limit()` is, again, basically the MongoDB drivers own `limit` function on the `EMongoQueryBuilder`
...
This validator was added as a easy, yet flexible, method to automate the conversion of hexidecimal representation of `MongoDB\BSON\ObjectID`s (for example: `addffrg33334455add0001`) to the 
`MongoDB\BSON\ObjectID` object for database manipulation. This validator can also handle arrays of strings that need converting to `MongoDB\BSON\ObjectID`s.
```php 
