...
class EMongoMessageSource extends CMessageSource
{
    ...
    protected function getDbConnection()
    {
        if ($this->_db !== null) {
            ...
            if (($this->_db = Yii::app()->getComponent($id)) instanceof CDbConnection) {
                ...
                Yii::t(
                        'yii', 'EMongoCache.connectionID "{id}" is invalid. Please make sure it refers to the ID of a EMongoClient application component.', ['{id}' => $id]
                )
                );
            }
        } else {
    ...
    /**
     * Returns current MongoDB\Collection object
     *
    ...
    protected function loadMessagesFromDb($category, $language)
    {
        ...
        $messages = json_decode(json_encode(iterator_to_array($this->getCollection()->find($criteria, $fields))));
