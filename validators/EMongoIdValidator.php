    ...
    protected function validateAttribute($object, $attribute)
    {
        ...
        if (is_array($value)) {
            foreach ($value as $key => $attr) {
                $value[$key] = $attr instanceof MongoDB\BSON\ObjectID ? $attr : new MongoDB\BSON\ObjectID($attr);
            }
            $object->$attribute = $value;
        } else {
            $object->$attribute = $object->$attribute instanceof MongoDB\BSON\ObjectID ? $object->$attribute : new MongoDB\BSON\ObjectID($object->$attribute);
        }
    }
